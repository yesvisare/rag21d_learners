...
class SmartRetriever:
    ...
    def _mock_results(self, query: str, top_k: int) -> List[RetrievalResult]:
        """Generate mock results when API unavailable."""
        return [
            RetrievalResult(
                id=f"mock-{i}",
                score=0.85 - (i * 0.05),
...
class ContextBuilder:
    ...
    def build_context(self, results: List[RetrievalResult]) -> str:
        ...
        for result in results:
            text_key = result.text.strip()[:100]  # Use first 100 chars as key
