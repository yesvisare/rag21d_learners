...
from src.m1_2_pinecone_hybrid.config import get_clients, OPENAI_MODEL, PINECONE_INDEX, DEFAULT_NAMESPACE, SCORE_THRESHOLD
...
def build_index(dimension: int = 1536, metric: str = "dotproduct"):
    ...
    # Check metric - dotproduct required for alpha-weighted vector combination
...
def upsert_hybrid_vectors(
    ...
    for idx, doc in enumerate(docs):
        try:
            ...
            # Validate metadata - truncate text to avoid 40KB limit
            metadata = {"text": doc[:500], "source": "example_data"}
...
def safe_namespace_query(
    ...
    try:
        # Check namespace exists before querying to avoid silent failures
...
def smart_alpha_selector(query: str) -> float:
    ...
    Strategy:
    - Keyword-heavy queries (short, with numbers/codes) → lower alpha (0.3)
    - Semantic-heavy queries (long, with question words) → higher alpha (0.7)
    - Balanced queries → middle alpha (0.5)
